...
- All need libraries for interacting with the rhonix network (locally/testnet/mainnet) are included.
...
- Cloudflare workers for caching rholang exploratory deploys from the rhonix network.
...
1. Download the [nft-template](https://github.com/rholang/nft) ➜ For that go into vscode and there click on View ➜ Command Palette
   ![vscode1](public/images/docs/build/vscode/vscode-8.png)
2. Then type in: git clone.  
   ![vscode1](public/images/docs/build/vscode/vscode-9.png)
   ...
   ![vscode1](public/images/docs/build/sdk/sdk-12.png)  
...
3. Open a terminal in vscode (Terminal ➜ New Terminal). Then open a Command Prompt (not powershell)
   ![vscode1](public/images/docs/build/vscode/vscode-11.png)
...
![sdk-6](public/images/docs/build/sdk/sdk-6.png)
...
![sdk-7](public/images/docs/build/sdk/sdk-7.png)
...
![sdk-8](public/images/docs/build/sdk/sdk-8.png)
...
3. Go to the test file (here index.test.ts in the packages/app/\_\_tests\_\_ folder).  
   ...
   ![sdk-9](public/images/docs/build/sdk/sdk-9.png)
...
![sdk-10](public/images/docs/build/sdk/sdk-10.png)
...
4. Open the vscode terminal and go to jest (vite-demo) output. You can see the result of the exploratory-deploy or deploy of the rholang code from the local rnode instance.
   ![sdk-11](public/images/docs/build/sdk/sdk-11.png)
...
4. Go to settings of your project and change it to the following:
   ![sdk-14](public/images/docs/build/sdk/sdk-14.png)

   ![sdk-15](public/images/docs/build/sdk/sdk-15.png)
...
Cloudflare workers lets you deploy serverless code, which can be used for small code execution. Together with cloudlflare workers key-value store one can use it for having a very fast response time, when thousands of user are doing exploratory-deploys on the Rhonix network.
...
When the first user visits the website, then explore-deploys are send to the cloudflare worker. Every explore-deploy to the cloudlfare worker is stored in their key-value store. If this request was never fetched, then for the first time it will send a request to the rhonix network. The response is cached. When a second user is requesting the same explore-deploy rholang code, then cloudlflare is sending the cached response back, which is very fast. When some smart-contract has changed on the rhonix network (deploy), then the rnode instance will send to the cloudlflare-worker a notice, that the cache is not up-to-date anymore.

![sdk-13](public/images/docs/build/sdk/sdk-13.png)
