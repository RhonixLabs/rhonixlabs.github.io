...
- `--valid-after-block-number <arg>` At what block height your code will be active.
  ...
  Unless you are a validator in the Rhonix network or a private network, you will typically deploy Rholang to a known validator with the intent to add the contract to the blockchain.
...
The current cost for file store on the Rhonix Mainnet is the following:
