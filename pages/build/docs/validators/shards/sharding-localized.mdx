---
title: Sharding Via "Localized Processes"
description: Sharding Via "Localized Processes"
---
...
URIs are a class of special Rholang processes denoted by backticks (see grammar). A URI is valid if it has the form of a (non-empty) path delimited by forward slash ('/') and beginning with "rhonix://". The base prefix alone "rhonix://" is not a valid URI because its path is empty. For example, `rhonix://coop/contracts/examples/hello_world` is a valid URI. Note that the base prefix "rhonix://" could be swapped out to easily create private blockchains, or other "alt-chains" build on the same Rhonix platform technology.

### Prefixes of a Valid URI

A prefix of a valid URI is the URI given by truncating after any forward slash ('/') following the base prefix "rhonix://". For example, the prefixes of `rhonix://coop/contracts/examples/hello_world` are: `rhonix://`, `rhonix://coop`, `rhonix://coop/contracts` and `rhonix://coop/contracts/examples`. Note that a URI is not a prefix of itself.
...
```javascript
 ...
 new multiplier in { contract multiplier(@x, return) = { for(@y <- @`rhonix://coop/contracts/examples/resources/multiply_factor`){ return!(y \* x) } } } is localized.
```

### Local Group

All localized processes and names belong to a local group. A local group is identified by a valid URI or the base prefix "rhonix://". The local group of localized processes and names are determined by the following rules:
...
```javascript
{ contract multiplier(@x, return) = { for(@y <- @`rhonix://coop/contracts/examples/resources/multiply_factor`){ return!(y \* x) } } }
```

is identified by `rhonix://coop/contracts/examples/resources`.

The local group of

```javascript
@`rhonix://A/a`!(0) | @`rhonix://B/b`!(2)
```

is identified by "rhonix://". Note that in this example the process is actually separable, meaning it can be written as the parallel composition of localized processes with different local groups. As discussed below, an optimization to the sharding solution proposed here could be to identify such cases and treat each separate part independently.
...
The sharding solution proposed here is as follows. Each shard will be identified by a single local group which can be represented by valid URI and it will serve code which is localized to that local group or any local group it contains that is not served by another shard. For example, suppose that S is a shard with identifying local group `rhonix://root` and T is a shard with identifying local group `rhonix://root/specific`. Then S would serve processes with local groups like `rhonix://root` and `rhonix://root/contracts` while T would serve local groups like `rhonix://root/specific` and `rhonix://root/specific/resources`. Code which contained some processes that would need to be served by S and some that would need to be served by T would have to be served by both in a merge block (as discussed below).

The dominance hierarchy of the shards will follow the natural tree structure for local groups, i.e. if A, B, and C are local groups and A = B ∪ C (note that this also implies that A = A ∪ B and A = A ∪ C) then a shard serving A is dominant over a shard serving B or C. When code is deployed to the platform, it must run on all shards which serve the local groups it uses, therefore it will need to exist in a merge block which includes all of those shards. For example, if A, B and C are local groups as above and all of them are served by different shards then a contract that contains processes localized to B and C will have local group equal to A (because A = B ∪ C) and will need to join the platform via a merge block between all three shards serving those local groups. Note that the root of the local group tree is "rhonix://", which is not a valid URI and therefore not a shard. If two shards wish to communicate and the union of their corresponding local groups is "rhonix://" then they will need to use a merge block, but the dominance will also need to be specified since it cannot be inferred from the tree.
