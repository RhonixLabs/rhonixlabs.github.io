# Setup a Rhonix network on a single machine with docker-compose

This repository is a continuation of https://github.com/nzpr/rchain-docker-cluster.
It's purpose is to deploy Rhonix shard (plus useful services around it) on local machine using docked-compose. It might be useful for those who wish to test changes to RNode, to get familiar with how network operates and how to configure and start private network. The behaviour is equal to normal network with each node having dedicated machine.
...
1. [Rhonix](https://github.com/RhonixLabs/rhonix)
2. [PyRhonix](https://github.com/rchain/pyrchain)
...
Make sure shard is up and running and all nodes printed `Making a transition to Running state.`, which means Weaver instance is ready to accept connections.
